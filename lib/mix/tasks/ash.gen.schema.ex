defmodule Mix.Tasks.Ash.Gen.Schema do
  ...
  #{for attr <- Mix.Ash.Schema.valid_types(), do: "  * `#{inspect attr}`\n"}
  ...
  def copy_new_files(%Schema{context_app: ctx_app} = schema, paths, binding) do
    ...
    Mix.Ash.copy_from(paths, "priv/templates/phx.gen.schema", binding, files)

    if schema.migration? do
      migration_path = Mix.Ash.context_app_path(ctx_app, "priv/repo/migrations/#{timestamp()}_create_#{schema.table}.exs")
      Mix.Ash.copy_from paths, "priv/templates/phx.gen.schema", binding, [
